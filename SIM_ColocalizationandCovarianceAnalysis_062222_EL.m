...
inDir = 'C:\Users\esi_000\Documents\Garcea Lab\MATLAB\For Analysis\*.nd2';
Folder = 'C:\Users\esi_000\Documents\Garcea Lab\MATLAB\For Analysis';
...
SAVE_IMAGE = 1; %Will save separated and thresholded images to folder if =1. If =0, no images will be saved (speeds up analysis).
SAVE_PCC = 1; % Will save 2-column matrices for each PCC comparison (1=yes, 0=no).
...
disp(progress2)
    ...
    filename = fullfile(Folder, srcFiles(f).name);
    ...
    loop = num2str(f);
    slash = '\';
    if SAVE_IMAGE > 0 || SAVE_PCC > 0, ImageDir = fullfile(Folder,newF); else end;
    
I = bfopen(char(filename));
